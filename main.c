#include "eui.h"
#include <SDL3/SDL.h>
#include <SDL3/SDL_main.h>

#define ERROR "\033[;31mERROR\033[0m: "
#define WARN  "\033[;33mWARN\033[0m: "
#define INFO  "\033[;32mINFO\033[0m: "
#define DEBUG "\033[;35mDEBUG\033[0m: "

static SDL_Window *window = NULL;
static SDL_Renderer *renderer = NULL;
uint8_t keyStatus[256];
EUI_KEY ka,kw,ks,kd;
SDL_AppResult SDL_AppInit(void **appstate, int argc, char *argv[]){//初始化
  (void)appstate; // 未使用的参数
  SDL_SetAppMetadata("EUI","0.0.1","imxyyt");
  if (!SDL_Init(SDL_INIT_VIDEO)) {
    SDL_Log(ERROR"init: %s", SDL_GetError());
    return SDL_APP_FAILURE;
  }
  if (!SDL_CreateWindowAndRenderer("window", 128, 64, 0, &window, &renderer)) {
    SDL_Log(ERROR"window: %s", SDL_GetError());
    return SDL_APP_FAILURE;
  }
  SDL_Log(INFO"start");
  for(int i=0;i<argc;i++)SDL_Log(INFO"argv[%d]=%s",i,argv[i]);
  return SDL_APP_CONTINUE;
}
void SDL_AppQuit(void *appstate, SDL_AppResult result){//销毁处理
  (void)appstate; // 未使用的参数
  (void)result;   // 未使用的参数
  SDL_Log(INFO"run time %.2fs",SDL_GetTicks()/1000.0);
}
void updateOLED(){
  for(int i=0;i<EUI_H>>3;i++){
    for(int j=0;j<8;j++){
      for(int k=0;k<EUI_W;k++){
        if(EUI_buf[i][k]&(1<<j)){
          SDL_SetRenderDrawColorFloat(renderer, 0,0,0, 0x00);
        }else{
          SDL_SetRenderDrawColorFloat(renderer, 0xff,0xff,0xff, 0x00);
        }
        SDL_RenderPoint(renderer,k,(i<<3)+j);
      }
    }
  }
  SDL_Delay(10);
  eui_update_key(keyStatus[SDLK_W],&kw);
  eui_update_key(keyStatus[SDLK_A],&ka);
  eui_update_key(keyStatus[SDLK_S],&ks);
  eui_update_key(keyStatus[SDLK_D],&kd);
}

SDL_AppResult SDL_AppIterate(void *appstate){//刷新帧
  (void)appstate; // 未使用的参数
  static Uint64 fpsStart=0;
  static Uint16 fps=1;//帧数能超65535算你牛逼
  static int16_t x=0,y=0,rx=16,ry=16,rw=20,rh=20;
  Uint64 nowTick=SDL_GetTicks();
  SDL_SetRenderDrawColorFloat(renderer, 0xff,0xff,0xff, 0x00);
  SDL_RenderClear(renderer);
  
  eui_clear(0);
  eui_puts(x,y,eui_i2str(x));
  eui_puts(x,y+8,eui_i2str(y));
  eui_set_focus(rx,ry);
  eui_circle(10,0b10101010);
  eui_rect(rw,rh,0);
  updateOLED();
  eui_set_focus(0,EUI_H-8);
  EUI_ON(kw,EUI_KEY_MODE_SHORT,{
    SDL_Log(INFO"W");
    y--;
  });
  EUI_ON(kw,EUI_KEY_MODE_LONG,{
    SDL_Log(INFO"lw");
    ry--;
  });
  EUI_ON(ka,EUI_KEY_MODE_SHORT,{
    SDL_Log(INFO"A");
    x--;
  });
  EUI_ON(ka,EUI_KEY_MODE_LONG,{
    SDL_Log(INFO"la");
    rx--;
  });
  EUI_ON(ks,EUI_KEY_MODE_SHORT,{
    SDL_Log(INFO"S");
    y++;
  });
  EUI_ON(ks,EUI_KEY_MODE_LONG,{
    SDL_Log(INFO"ls");
    ry++;
  });
  EUI_ON(kd,EUI_KEY_MODE_SHORT,{
    SDL_Log(INFO"D");
    x++;
  });
  EUI_ON(kd,EUI_KEY_MODE_LONG,{
    SDL_Log(INFO"ld");
    rx++;
  });
  
  SDL_RenderPresent(renderer);
  if(nowTick-fpsStart>=1000){
    SDL_Log(INFO"%u fps",fps);
    fpsStart=nowTick;
    fps=0;
  }
  fps++;
  return SDL_APP_CONTINUE;
}
SDL_AppResult SDL_AppEvent(void *appstate, SDL_Event *event){//事件处理
  (void)appstate; // 未使用的参数
  switch (event->type){
    case SDL_EVENT_QUIT://退出
      return SDL_APP_SUCCESS;
    case SDL_EVENT_KEY_DOWN://按键按下
      if(event->key.key<256) keyStatus[event->key.key]=1;
      return SDL_APP_CONTINUE;
    case SDL_EVENT_KEY_UP://按键抬起
      if(event->key.key<256) keyStatus[event->key.key]=0;
      return SDL_APP_CONTINUE;
    default:
      return SDL_APP_CONTINUE;
  }
}