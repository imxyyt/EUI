CC = gcc
CFLAGS = -Wall -Wextra -pedantic -std=c23 -g -fsanitize=address `pkg-config --cflags sdl3 sdl3-ttf sdl3-image` -DSDL_MAIN_USE_CALLBACKS
LDFLAGS =  -fsanitize=address `pkg-config --libs sdl3 sdl3-ttf sdl3-image`

BUILD_DIR=build/
SRC = $(wildcard src/*.c src/*/*.c) main.c
INC = src
EXEC = EUI
#下方代码最好不要更改
